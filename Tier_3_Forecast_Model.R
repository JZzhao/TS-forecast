...
case_select<-read.csv("./step_2/Segments_for_Step_3.csv")
...
if(nrow(short_case)>=1){
  ...
  write.csv(short_case,'./Step_3/Short_case_Less_Than_24_Months.csv',row.names = FALSE)
}
...
forecast_methods<-c('forecast_enhance_stl','forecast_enhance_stl_fit')
for (product in unique(model_data_complete$Product.Category)){
  accuracy_wb <- createWorkbook('xlsx')
  for (base in unique(model_data_complete[model_data_complete$Product.Category==product,'Base_Feature'])){
    ...
    temp_sheet<-createSheet(accuracy_wb, sheetName=base)
    addDataFrame(accuracy_record , sheet=temp_sheet, startColumn=1, row.names=FALSE)
  }
  saveWorkbook(accuracy_wb, paste0("./Step_3/Step_3_",product,'_',"Accuracy_ALL_Models",suffix,".xlsx"))
}
...
for (product in unique(model_data_complete$Product.Category)){
  for (base in unique(model_data_complete[model_data_complete$Product.Category==product,'Base_Feature'])){
    accuracy_record <-openxlsx::read.xlsx(paste0("./Step_3/Step_3_",product,'_',"Accuracy_ALL_Models",suffix,".xlsx"),sheet = base)
...
for (product in unique(accuracy_rank$Product.Category)){
  ...
  xlsx::saveWorkbook(backtest_wb, paste0("./Step_3/Step_3_",product,'_',"Backtest_record",suffix,".xlsx"))
}
...
for (product in unique(accuracy_rank$Product.Category)){
  ...
  xlsx::saveWorkbook(forecast_wb, paste0("./Step_3/Step_3_",product,'_',"Forecast_record",suffix,".xlsx"))
}
...
for (product in unique(model_data_complete$Product.Category)){
  for (base in unique(model_data_complete[model_data_complete$Product.Category==product,'Base_Feature'])){
    ...
    ###=====1.Metric: Accuracy=======###
    accuracy_record <-xlsx::read.xlsx(paste0("./Step_3/Step_3_",product,'_',"Backtest_record",suffix,".xlsx"),sheetName = 'Accuracy')
    ...
    fcst_df <-xlsx::read.xlsx(paste0("./Step_3/Step_3_",product,'_',"Forecast_record",suffix,".xlsx"),sheetName = sheet)
    ...
    for (col in colnames(fcst_df)) {
      ...
      method<-ifelse(grepl('fit',col),'forecast_enhance_stl_fit','forecast_enhance_stl')
...
write.csv(best_method,paste0('./Step_3/Step_3_master_file_backup',suffix,'.csv'),row.names = F)
if(nrow(best_method[best_method$Accuracy>=accuracy_threshold,])>=1){
  write.csv(best_method[best_method$Accuracy>=accuracy_threshold,],paste0('./Step_3/Step_3_Best_Method',suffix,'.csv'),row.names = F)
}

if(nrow(best_method[best_method$Accuracy<accuracy_threshold,])>=1){
  write.csv(best_method[best_method$Accuracy<accuracy_threshold,],paste0('./Step_3/Segments_for_Step_4',suffix,'.csv'),row.names = F)
}
...
dir.create(paste0("./Step_3/Step_3_pic_output",suffix))
for (product in unique(model_data_complete$Product.Category)){
  ...
  for (base in unique(model_data_complete[model_data_complete$Product.Category==product,'Base_Feature'])){
    ...
    bk_df <-xlsx::read.xlsx(paste0("./Step_3/Step_3_",product,'_',"Backtest_record",suffix,".xlsx"),sheetName = sheet)
    ...
    trend_sheet <- createSheet(trend_wb, sheet)
      ...
      for (i in 1:nrow(rank_list_tmp)){
        ...
        #Save the graph
        p<-try(ggsave(paste("./Step_3/Step_3_pic_output", suffix,"/", sheet, col, '.png', sep=''),
                      ggplot()+
        ...
        pic_file <- paste("./Step_3/Step_3_pic_output", suffix, "/",sheet, col, '.png',sep='')
  ...
  xlsx::saveWorkbook(trend_wb,paste0("./Step_3/Step_3_",product,'_', "Forecast_Summary_Plot",suffix,".xlsx"))
}
